...
void *message_listener(void *vargp) {
    ...
    for (;;) {
        ...
        //fprintf(stderr, "[Exchange     ] message_listener(): ex_entity=%p, cse_req.base=%p, cse_req_arena=%p, cse_res.base=%p, cse_res_arena=%p\n", &ex_entity, &cse_req.base_, &cse_req_arena, &cse_res.base_, &cse_res_arena);
